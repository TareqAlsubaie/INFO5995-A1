...
public abstract class AbstractSendChannel<E> implements SendChannel<E> {
    ...
    */
    ...
    public final Object mo1655trySendJP2dKIU(E element) {
        Object objOfferInternal = offerInternal(element);
        if (objOfferInternal == AbstractChannelKt.OFFER_SUCCESS) {
            return ChannelResult.INSTANCE.m1675successJP2dKIU(Unit.INSTANCE);
        }
        if (objOfferInternal == AbstractChannelKt.OFFER_FAILED) {
            Closed<?> closedForSend = getClosedForSend();
            return closedForSend == null ? ChannelResult.INSTANCE.m1674failurePtdJZtk() : ChannelResult.INSTANCE.m1673closedJP2dKIU(helpCloseAndGetSendException(closedForSend));
        }
        if (objOfferInternal instanceof Closed) {
            return ChannelResult.INSTANCE.m1673closedJP2dKIU(helpCloseAndGetSendException((Closed) objOfferInternal));
        }
    ...
    public final void helpCloseAndResumeWithSendException(Continuation<?> continuation, E e, Closed<?> closed) {
        ...
        if (function1 != null && (undeliveredElementExceptionCallUndeliveredElementCatchingException$default = OnUndeliveredElementKt.callUndeliveredElementCatchingException$default(function1, e, null, 2, null)) != null) {
            ...
            continuation.resumeWith(Result.m147constructorimpl(ResultKt.createFailure(undeliveredElementException)));
            return;
        }
        Result.Companion companion2 = Result.INSTANCE;
        continuation.resumeWith(Result.m147constructorimpl(ResultKt.createFailure(sendException)));
    }
    ...
    */
    ...
    private final void helpClose(Closed<?> closed) {
        ...
        while (true) {
            ...
            if (receive == null) {
                break;
            } else if (!receive.mo1699remove()) {
                receive.helpRemove();
            } else {
                objM1691constructorimpl$default = InlineList.m1696plusFjFbRPM(objM1691constructorimpl$default, receive);
            }
        }
        if (objM1691constructorimpl$default != null) {
            if (!(objM1691constructorimpl$default instanceof ArrayList)) {
                ((Receive) objM1691constructorimpl$default).resumeReceiveClosed(closed);
            } else if (objM1691constructorimpl$default != null) {
                ArrayList arrayList = (ArrayList) objM1691constructorimpl$default;
