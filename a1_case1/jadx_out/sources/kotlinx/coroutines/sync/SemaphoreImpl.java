...
final class SemaphoreImpl implements Semaphore {
    ...
    public final boolean addAcquireToQueue(CancellableContinuation<? super Unit> cont) {
        Object objM1703constructorimpl;
        boolean z;
            ...
            while (true) {
                if (segment2.getId() < j || segment2.getRemoved()) {
                    ...
                    if (obj == ConcurrentLinkedListKt.CLOSED) {
                        objM1703constructorimpl = SegmentOrClosed.m1703constructorimpl(ConcurrentLinkedListKt.CLOSED);
                        break;
                    }
                    ...
                    objM1703constructorimpl = SegmentOrClosed.m1703constructorimpl(segment2);
                    break;
                }
            }
            if (SegmentOrClosed.m1708isClosedimpl(objM1703constructorimpl)) {
                break;
            }
            Segment segmentM1706getSegmentimpl = SegmentOrClosed.m1706getSegmentimpl(objM1703constructorimpl);
            while (true) {
                Segment segment3 = (Segment) this.tail;
                if (segment3.getId() >= segmentM1706getSegmentimpl.getId()) {
                    break;
                }
                if (!segmentM1706getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                    z = false;
                    break;
                }
                if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(tail$FU, this, segment3, segmentM1706getSegmentimpl)) {
                    if (segment3.decPointers$kotlinx_coroutines_core()) {
                        segment3.remove();
                    }
                } else if (segmentM1706getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
                    segmentM1706getSegmentimpl.remove();
                }
            }
            z = true;
        } while (!z);
        SemaphoreSegment semaphoreSegment = (SemaphoreSegment) SegmentOrClosed.m1706getSegmentimpl(objM1703constructorimpl);
    ...
    private final boolean tryResumeNextFromQueue() {
        ...
        do {
            ...
            while (true) {
                ...
                if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(head$FU, this, segment3, segmentM1706getSegmentimpl)) {
                    if (segment3.decPointers$kotlinx_coroutines_core()) {
                        segment3.remove();
                    }
                } else if (segmentM1706getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
        ...
        if (andSet == null) {
            ...
            return !ConcurrentWeakMap$Core$$ExternalSyntheticBackportWithForwarding0.m(semaphoreSegment.acquirers, i2, SemaphoreKt.PERMIT, SemaphoreKt.BROKEN);
        }
