...
public final class TestTimeSource extends AbstractLongTimeSource {
    ...
    public final void m1609plusAssignLRDsOJo(long duration) {
        long j;
        long jM1520toLongimpl = Duration.m1520toLongimpl(duration, getUnit());
        if (jM1520toLongimpl != Long.MIN_VALUE && jM1520toLongimpl != Long.MAX_VALUE) {
            long j2 = this.reading;
            j = j2 + jM1520toLongimpl;
            if ((jM1520toLongimpl ^ j2) >= 0 && (j2 ^ j) < 0) {
                m1608overflowLRDsOJo(duration);
            }
        } else {
            double dM1517toDoubleimpl = this.reading + Duration.m1517toDoubleimpl(duration, getUnit());
            if (dM1517toDoubleimpl > 9.223372036854776E18d || dM1517toDoubleimpl < -9.223372036854776E18d) {
                m1608overflowLRDsOJo(duration);
            }
            j = (long) dM1517toDoubleimpl;
        }
        this.reading = j;
    }
